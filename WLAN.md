...
广域网WAN（Wide  Area NetWork）是覆盖较大地理范围的数据通信网络。WAN可能会覆盖一座城市、一个国家/地区或全球，一般情况广域网使用ISP（Internet Service Provider）提供的传输设施传输数据。


**广域网技术中，最常见的两种数据交换技术是电路交换和分组交换**
- 电路交换（Circuit Switching）  
采用电路交换技术进行数据传输期间，在源和目的节点之间有一条利用中间节点构成的专用物理连接线路，走到数据传输结束，这条物理线路才被释放其他通信所用。如果两个相邻节点之间的通信容量很大，那么这两个节点之间可以复用多条线路。用电路交换技完成数据传输，经历电路建立、数据传输、电路拆除三个过程。
- 分组交换（Packet Switching）  
将一个报文分成若干个级，每个分组的长度有一个上限，典型长度是数千个bit位。有限长度的分组使每个节点所需要的存储能力降低，提高了交换速度，分组交换用于交互工通信。
